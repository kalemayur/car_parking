...
class Parking
  ...
  def create_parking_lot(parking_lot_number)
    ...
    @parked_slots = {}
    puts "Created a parking lot with #{parking_lot_number} lots"
  end
  ...
  def park_car(params)
    ...
    p "Allocated lot number: #{available_slot}"
  end
  ...
  def leave(slot_number)
    if @parked_slots.key?(slot_number)
      ...
      p "lot number #{slot_number} is free"
    else
      return p "Sorry, Parking lot is not present"
    end
  end
  ...
  def get_register_number_for_color(color)
    ...
    return p "Slot is not available" if number.empty?
    p number
  end
  ...
  def get_slot_for_color(color)
    ...
    return p "Slot is not available" if get_slot.empty?
    p get_slot
  end
